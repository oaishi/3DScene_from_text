...
def train_video(target_tensor_color, target_tensor_shape, target_tensor_texture, target_tensor_size, target_tensor_motion, 
...
def trainIters(dataloader, decoder, decoder_optimizer,n_iters, print_every=50, plot_every=100, learning_rate=0.01, save_every =130):
    ...
    weight_shape = torch.tensor([1.0,0.91,1.0,1.0,1.0], device=device)
    weight_size = torch.tensor([1.0,1.37,1.0,1.0], device=device)
    weight_mat = torch.tensor([1.0,1.37,1.0,1.0], device=device)
    ...
    for epoch in range(0, n_iters + 1):
        for iter, sample_batched in enumerate(dataloader):
            ...
            loss, acc = train_video(color_list, shape_list, texture_list, size_list, motion_list,
    ...
    #                     print('model saved at:', model_save_path)
        torch.save(state, 'model_weight_video/model_%d_%d' % (epoch, iter))
