...
def start_train(type, checkpoint_file = None):
    hidden_size = 1024
    ...
    if type == "image":   
        ...
        train_image.trainIters(dataloader, attn_decoder1, decoder_optimizer,5000) 
    if type == "video":
        output_folder = '../per_video_file/train/'    
        dataloader = getdataloader(type, batch_size,output_folder)
        train_video.trainIters(dataloader, attn_decoder1, decoder_optimizer,5000) 
    if type == "combined":
        output_folder = '../per_description_file/train/'
        video_folder = '../per_video_file/train/'    
        dataloader = getdataloader(type, batch_size,output_folder, video_folder)      
        train_full.trainIters(dataloader, attn_decoder1, decoder_optimizer,5000) 
