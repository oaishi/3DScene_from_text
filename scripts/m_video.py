...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size_color, output_size_shape, output_size_mat,
                 output_size_size, output_size_motion, dropout_p=0.1, max_length=MAX_WORD_LENGTH,batch_size= BATCH_SIZE):
        ...
        self.output_size_motion = output_size_motion
        ...
        self.embedding_mat = nn.Embedding(self.output_size, 204)
        self.embedding_size = nn.Embedding(self.output_size, 204)
        self.embedding_motion = nn.Embedding(self.output_size, 208)
...
#         self.attn_combine = nn.Linear(self.hidden_size * 2, self.hidden_size)
        ...
        self.out_motion = nn.Linear(self.hidden_size, self.output_size_motion)

    def forward(self, input_color, input_shape, input_size, input_mat, input_motion, hidden, cell_state, encoder_outputs):
        ...
        # print("inside-train-input", input_color.size())
        embedded_motion = self.embedding_motion(input_motion).view(1, self.batch_size, 1, -1)
        embedded_motion = self.dropout(embedded_motion)
        
        # print('embedding-layer-output-size', embedded_color.size(), embedded_shape.size(), embedded_size.size(), embedded_mat.size())
        
        embedded = torch.cat((embedded_color[0], embedded_shape[0], embedded_size[0], embedded_mat[0], embedded_motion[0]), 2)
...
#         print('attn-weights', attn_weights.size(), encoder_outputs.size())
        ...
        output_motion = F.log_softmax(self.out_motion(output), dim=1)
        
        
        return output_color, output_shape, output_material, output_size, output_motion, hidden, cell_state, attn_weights  